...
Updated: 2026-09-22. This is a decision list, not a legal compliance certificate. The product is currently free and used to test market demand. The legal pages are drafts and must be reviewed by an Israeli lawyer before launch.
...
- [ ] Verify the actual storage and processing countries, transfer mechanism, and current subprocessor terms for Vercel, Convex, Google OAuth/Maps, OpenAI, PostHog, and Sentry. No email service is implemented in this repository.
- [ ] Decide whether marketing emails will be offered. The application currently stores a separate optional preference but sends no marketing email.
...
Code review found Vercel, Convex, Google OAuth, Google Maps, OpenAI, PostHog and Sentry integrations. No email sending provider was found. The site offers a data-copy request by email and self-service deletion of user-linked Convex data; complete erasure across all providers is not implemented. A current retention policy and backup restore proof do not exist in the repository.
