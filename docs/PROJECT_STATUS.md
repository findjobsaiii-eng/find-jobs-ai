...
- Automatic provider calls fail closed behind a kill switch, daily run/query ceilings, concurrency ceiling, and a configurable output-token cap. The SDK retries transient connection, timeout, rate-limit, and server failures up to twice before the existing scheduler-level retry takes over; usage and bounded failure diagnostics are recorded.
