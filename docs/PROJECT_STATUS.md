...
- Owner-scoped tracking snapshots support saved jobs, applied, recruiter contact, phone screen, interview, assignment, final interview, offer, rejected, and withdrawn stages. Every card has a footer Save/status picker with colored status icons and an adjacent comment action; tracked cards expose removal as the final picker option. Choosing a new status opens an optional-comment dialog. Status changes and standalone bounded private notes appear in a compact dated timeline below Key skills, including comments attached to a status change. Closed job postings remain in history with an unavailable label.
