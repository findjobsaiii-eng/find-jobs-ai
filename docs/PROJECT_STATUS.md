...
- Owner-scoped tracking snapshots support saved jobs, applied, recruiter contact, phone screen, interview, assignment, final interview, offer, rejected, and withdrawn stages. A footer status picker replaces the separate save and “Sent résumé” controls. Status changes and standalone bounded private notes are retained as dated timeline events, including optional comments attached to a status change. Closed job postings remain in history with an unavailable label.
